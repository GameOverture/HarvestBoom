...
class Tile : public HyEntity2d
{
	...
	public:
		...
		{
			m_ProgressBarOutline.shape.SetAsBox(52.0f, 4.0f);
			m_ProgressBarOutline.SetTint(HyColor(0.0f, 0.0f, 0.0f));
			m_ProgressBarOutline.SetDisplayOrder(DISPLAYORDER_ProgressBar);

			m_ProgressBarFill.shape.SetAsBox(50.0f, 2.0f);
		...
		void SetColor_Tilling() {
			m_ProgressBarFill.SetTint(HyColor(1.0f, 0.4f, 0.5f));
		}
		void SetColor_Planting() {
			m_ProgressBarFill.SetTint(HyColor(170, 216, 143));
		}
		void SetColor_Growing() {
			m_ProgressBarFill.SetTint(HyColor(250, 248, 153));
		}
		void SetColor_Harvesting() {
			m_ProgressBarFill.SetTint(HyColor(230, 140, 86));
		}
		...
		void OffsetPercent(float fPercentOffset) {
			...
			SetVisible(m_ProgressBarFill.scale.X() < 1.0f);
		}

		void SetPercent(float fPercent) {
			m_ProgressBarFill.scale.X(HyClamp(fPercent, 0.0f, 1.0f));
			//SetVisible(m_ProgressBarFill.scale.X() < 1.0f && m_ProgressBarFill.scale.X() != 0.0f);
		}
	};
