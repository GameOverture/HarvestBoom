...
void Bug::SetHealth(float fHealth)
{
	...
	SetTint(HyColor(1.0f, fHealth, fHealth));
}
...
void Bug::InterruptWalkTo(float fDuration)
{
	...
	float fRemainingDuration = pos.GetAnimRemainingDuration();
	glm::vec2 ptTweenDest = pos.GetAnimDestination();
	pos.StopAnim();
...
void Bug::BugUpdate()
{
	m_fDeferTimer = HyMax(m_fDeferTimer - HyEngine::DeltaTime(), 0.0f);
	...
	{
		m_Leaf.SetAnimRate(5.0f);
		m_Leaf.SetAnimPause(false);

		if(m_Leaf.rot.IsAnimating() == false)
		{
	...
	else
	{
		...
		{
			if(m_Leaf.rot.Get() != 0.0f && m_Leaf.rot.IsAnimating() == false)
				m_Leaf.rot.Tween(0.0f, 0.75f, HyTween::QuadInOut);

			m_Leaf.SetAnimPause(true);
		}
	}

	if(m_ptPrevPos != pos.Get() && rot.IsAnimating() == false)
	{
