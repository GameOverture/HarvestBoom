...
Game::Game() :
	...
	m_fElapsedTime(0.0f)
{
	m_DebugGrid.GetText().pos.Set(HyEngine::Window().GetWindowSize().x - 25, HyEngine::Window().GetWindowSize().y - 25);
	m_DebugGrid.SetVisible(false);
	m_DebugGrid.SetDisplayOrder(DISPLAYORDER_DebugGrid);
	

	HyCamera2d *pCam = HyEngine::Window().GetCamera2d(0);
	pCam->pos.Set(static_cast<int>(m_Player.pos.X() * 2.0f), static_cast<int>(m_Player.pos.Y() * 2.0f));
	pCam->SetZoom(2.0f);

	m_HousePanel.Show(true);
	m_HousePanel.Hide(true);
}
...
void Game::GameUpdate()
{
	...
	case GAMESTATE_Bugs: {
		...
		if(HyEngine::Input().IsActionDown(MoveUp))
			pCam->pos.Offset(0.0f, fCameraPanSpeed * HyEngine::DeltaTime());
		else if(HyEngine::Input().IsActionDown(MoveDown))
			pCam->pos.Offset(0.0f, -fCameraPanSpeed * HyEngine::DeltaTime());

		if(HyEngine::Input().IsActionDown(MoveLeft))
			pCam->pos.Offset(-fCameraPanSpeed * HyEngine::DeltaTime(), 0.0f);
...
{
	if(HyEngine::Input().IsActionReleased(ToggleFPS))
	{
		if(HyEngine::Diagnostics().GetShowFlags() == 0)
			HyEngine::Diagnostics().Show(HYDIAG_ALL);
		else
			HyEngine::Diagnostics().Show(HYDIAG_NONE);
	}

	// Debug Grid
	if(HyEngine::Input().IsActionReleased(ToggleGrid))
	...
	m_DebugGrid.GetText().SetText(sDebugText);
}
