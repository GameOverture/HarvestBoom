...
/*virtual*/ void DayNight::OnLoaded() /*override*/
{
	...
	m_DayNight.shape.SetAsBox(vWindowSize.x, vWindowSize.y);
...
/*virtual*/ void DayNight::OnUpdate() /*override*/
{
	...
	case STATE_Off:
		break;
	case STATE_Intro:
		if(m_MainText.scale.IsAnimating() == false)
		{
	...
	case STATE_Cycling:
		OffsetTime(HyEngine::DeltaTime());
		if(Values::Get()->m_fDAY_LENGTH - m_fTime < 10.0f)
		{
			m_MainText.pos.Y(HyEngine::Window().GetWindowSize().y - 100.0f);
			m_MainText.SetVisible(true);
			m_MainText.alpha.Set(1.0f);
			m_MainText.scale.Set(1.0f, 1.0f);
			m_MainText.SetText("10");
	...
	case STATE_CountDown:
		OffsetTime(HyEngine::DeltaTime());
		m_MainText.SetText(std::to_string(1 + static_cast<int32>(Values::Get()->m_fDAY_LENGTH - m_fTime)));

		if(m_fTime == Values::Get()->m_fDAY_LENGTH)
		{
			m_MainText.SetText("Finish!");
			m_MainText.pos.Set(HyEngine::Window().GetWindowSize().x * 0.5f, HyEngine::Window().GetWindowSize().y * 0.5f);
			m_MainText.scale.Set(0.001f, 0.001f);
	...
	case STATE_Outro:
		if(m_MainText.scale.IsAnimating() == false && m_MainText.alpha.Get() == 1.0f)
		{
