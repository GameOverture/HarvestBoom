	...
	m_fEquipIdleHeight(0.0f)
{
	...
	m_Tool.shape.SetAsBox(5.0f, 30.0f);
	m_Tool.pos.Set(10.0f, 25.0f);
	m_Tool.rot.Set(30.0f);

	m_Collision.shape.SetAsBox(PLAYER_WIDTH, PLAYER_HEIGHT);
...
{
	return m_Collision.GetSceneTransform();
}
...
void Player::HandleInput(Tile *pPlayerTile)
{
	if(HyEngine::Input().IsActionDown(MoveUp))
		m_vVelocity.y = HyClamp(m_vVelocity.y + (HyEngine::DeltaTime() * (m_vVelocity.y < 0.0f ? Values::Get()->m_fPLAYER_DECEL : Values::Get()->m_fPLAYER_ACCEL)), m_vVelocity.y, Values::Get()->m_fPLAYER_MAXVELOCITY);
	else if(HyEngine::Input().IsActionDown(MoveDown))
		m_vVelocity.y = HyClamp(m_vVelocity.y - (HyEngine::DeltaTime() * (m_vVelocity.y > 0.0f ? Values::Get()->m_fPLAYER_DECEL : Values::Get()->m_fPLAYER_ACCEL)), -Values::Get()->m_fPLAYER_MAXVELOCITY, m_vVelocity.y);
	else
	{
		if(m_vVelocity.y > 0.0f)
			m_vVelocity.y = HyClamp(m_vVelocity.y - (HyEngine::DeltaTime() * Values::Get()->m_fPLAYER_DECEL), 0.0f, m_vVelocity.y);
		else if(m_vVelocity.y < 0.0f)
			m_vVelocity.y = HyClamp(m_vVelocity.y + (HyEngine::DeltaTime() * Values::Get()->m_fPLAYER_DECEL), m_vVelocity.y, 0.0f);
	}

	if(HyEngine::Input().IsActionDown(MoveLeft))
		m_vVelocity.x = HyClamp(m_vVelocity.x - (HyEngine::DeltaTime() * (m_vVelocity.x > 0.0f ? Values::Get()->m_fPLAYER_DECEL : Values::Get()->m_fPLAYER_ACCEL)), -Values::Get()->m_fPLAYER_MAXVELOCITY, m_vVelocity.x);
	else if(HyEngine::Input().IsActionDown(MoveRight))
		m_vVelocity.x = HyClamp(m_vVelocity.x + (HyEngine::DeltaTime() * (m_vVelocity.x < 0.0f ? Values::Get()->m_fPLAYER_DECEL : Values::Get()->m_fPLAYER_ACCEL)), m_vVelocity.x, Values::Get()->m_fPLAYER_MAXVELOCITY);
	else
	{
		if(m_vVelocity.x > 0.0f)
			m_vVelocity.x = HyClamp(m_vVelocity.x - (HyEngine::DeltaTime() * Values::Get()->m_fPLAYER_DECEL), 0.0f, m_vVelocity.x);
		else if(m_vVelocity.x < 0.0f)
			m_vVelocity.x = HyClamp(m_vVelocity.x + (HyEngine::DeltaTime() * Values::Get()->m_fPLAYER_DECEL), m_vVelocity.x, 0.0f);
	}
	...
	pos.Offset(m_vVelocity * HyEngine::DeltaTime());
	float fMagnitude = GetMagnitude();
	if(abs(fMagnitude) > 0.0f)
	{
		m_Body.SetState(1);
		m_Body.SetAnimRate(fMagnitude / Values::Get()->m_fPLAYER_MAXVELOCITY);
	}
	else
		m_Body.SetState(0);

	//m_DebugText.TextSet(std::to_string(fMagnitude));
	//m_DebugText.TextSet(std::to_string(HyEngine::Window().GetCamera2d(0)->pos.Y()));
}
