											...
											m_DebugText(HY_SYSTEM_FONT, this)
{
	...
	m_DebugText.SetTextAlignment(HYALIGN_Right);
	...
	for(int32 i = -DEBUG_GRID_SIZE; i < DEBUG_GRID_SIZE; ++i)
	{
		...
		m_DebugGridHorz.back()->shape.SetAsLineSegment(glm::vec2(-1000.0f, i * TILE_SIZE), glm::vec2(1000.0f, i * TILE_SIZE));
		if(i == 0)
		...
		m_DebugGridVert.back()->shape.SetAsLineSegment(glm::vec2(i * TILE_SIZE, -1000.0f), glm::vec2(i * TILE_SIZE, 1000.0f));
		if(i == 0)
