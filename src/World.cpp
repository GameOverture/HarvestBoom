	...
	m_DebugCollideNormal(nullptr)
{
	...
	m_DebugCollidePt1.SetAsCircle(2.0f);
	...
	m_DebugCollidePt2.SetAsCircle(2.0f);
...
void World::UpdatePlayer(Player &playerRef, Stamina &staminaRef, HousePanel &housePanelRef)
{
	...
	if(pPlayerTile)
	{
		...
		if(Hy_Input().IsActionDown(UseEquip) && pPlayerTile->GetTileType() == Dirt)
		{
	...
	for(uint32 i = 0; i < WORLD_WIDTH; ++i)
	{
		for(uint32 j = 0; j < WORLD_HEIGHT; ++j)
		{
			...
			if(bColliding)
			{
				...
				m_DebugCollideNormal.SetAsLineSegment(ptMidpoint, ptMidpoint + vNormal);
				m_DebugCollideNormal.Load();
