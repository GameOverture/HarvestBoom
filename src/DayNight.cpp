...
/*virtual*/ void DayNight::OnLoaded() /*override*/
{
	...
	m_DayNight.SetAsBox(vWindowSize.x, vWindowSize.y);
...
/*virtual*/ void DayNight::OnUpdate() /*override*/
{
	...
	case STATE_Cycling:
		...
		{
			m_MainText.pos.Y(Hy_Window().GetWindowSize().y - 100.0f);
			m_MainText.SetVisible(true);
	...
	case STATE_CountDown:
		...
		if(m_fTime == Values::Get()->m_fDAY_LENGTH)
		{
			...
			m_MainText.pos.Set(Hy_Window().GetWindowSize().x * 0.5f, Hy_Window().GetWindowSize().y * 0.5f);
			m_MainText.scale.Set(0.001f, 0.001f);
